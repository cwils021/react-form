...
const useForm = () => {
  ...
  const techVisible = useCallback(() => {
    ...
    if (values.role && values.role === "Fullstack Engineer") {
      ...
      values.role &&
      values.role === "Product Manager" &&
      values.industry === "Software development"
    ) {
      visible = true;
    } else {
      if (values.technical_requirement) {
        delete values.technical_requirement;
      }
